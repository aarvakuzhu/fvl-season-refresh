...
const { Team, Standing, Season, CoreMember, Decision } = require('./models');
...
async function seed() {
  ...
  // Link standings to team IDs
  const teamMap = {};
  insertedTeams.forEach(t => { teamMap[t.name] = t._id; });
  const standingsWithIds = standings.map(s => ({ ...s, teamId: teamMap[s.teamName] }));
  await Standing.insertMany(standingsWithIds);
  console.log(`Inserted ${standingsWithIds.length} standings`);

  await Season.create(season);
  console.log('Inserted season config');

  await CoreMember.insertMany(coreMembers);
  console.log(`Inserted ${coreMembers.length} core members`);
